...
![image-20230901092349390](https://img-blog.csdnimg.cn/5815bdf296134c8ab58e98a865b6e1e5.png)
...
![image-20230901101558329](https://img-blog.csdnimg.cn/209dd4f8f5ac4687ad0de9a89caf9a67.png)
...
![image-20230901103020737](https://img-blog.csdnimg.cn/11c35c8b4b3b4f48879ab3cb66b67ac3.png)
...
![image-20230901111815358](https://img-blog.csdnimg.cn/fd0cb3fa2a544890954403e49dabbf88.png)
...
![image-20230901113952445](https://img-blog.csdnimg.cn/e95780f38cb14cb489fd6d4b37367eaf.png)
...
- **如果`oldpath`是一个文件而不是目录，那么就对该文件本身进行重命名，对于符号链接也是如此，对符号链接本身进行命名**
  ...
  **在这种情况下，`newpath`肯定最好是一个目录里不存在的文件名，如果已存在，`newpath`肯定不能为一个目录，假如我们想要命名的`newpath`已经存在，就是重名，那么内核会将`newpath`的目录项删除并且将`oldpath`的文件重命名为`newpath`，这意味着原来叫`newpath`的文件会被删除，然后被替代**
  ...
  <img src="https://img-blog.csdnimg.cn/fe6d089286974ef094050db720029c50.png" alt="image-20230901132559546" style="zoom:80%;" />

  执行过后，结果：

  原`08.txt`已经被删掉，然后`08_1.txt`重命名为了`08.txt`

  ![image-20230901132633366](https://img-blog.csdnimg.cn/98cc223f69924afe9694617cbe8be767.png)

  ![image-20230901132653264](https://img-blog.csdnimg.cn/180f0eb4e2004d9fb57c2d858e781b54.png)
...
- **如果`oldpath`是一个目录，`newpath`肯定最好也是一个名字不重复的新目录名字；如果重名了，那么这个目录需要是一个空目录才行，这时就会先把空目录删除然后将`oldpath`重命名，对于有内容的目录是不行的**
  ...
  **不能对`./`目录和`../`目录重命名**，比如我不能打开一个目录，然后使用`./`给这个目录重命名，不可以的
  ...
  ![image-20230901144324802](https://img-blog.csdnimg.cn/2e46b5ce7fb047158351c9ab0d8b8e4b.png)

  当然我们也要试一下重命名成为一个有文件的目录，这里我创建一个目录`test2`，`test`目录里存放了一些文件

  这是原来的目录tree：

  ![image-20230901144650852](https://img-blog.csdnimg.cn/c76df7ba1f0e4ee99825c8e173f95921.png)
  ...
  ![image-20230901144731958](https://img-blog.csdnimg.cn/938ab7283b8b4f68aabb2fd38490c7ff.png)

  现在我让`test2`加上数据，再来尝试：

  ![image-20230901144925401](https://img-blog.csdnimg.cn/23816801d3384dbf97c9a2f6ca5a6ccd.png)

  执行结果：

  函数返回了-1，错误信息说目录不为空，可见结果在预期内

  ![image-20230901144951938](https://img-blog.csdnimg.cn/4ccf18183dee411fa2e89f4579f871d4.png)
...
- **在底层文件系统支持的前提下，只有超级用户能够创建指向目录的硬链接，其他的是不允许的，我的系统就不行**
  ...
  ![image-20230901151254437](https://img-blog.csdnimg.cn/fc20b36870c74b49bcb4df517d07b310.png)
  ...
  ![image-20230901151357940](https://img-blog.csdnimg.cn/1bcde1cb1993433ba9538b7e8192acba.png)
...
<img src="https://img-blog.csdnimg.cn/90db3145628a4f61a4dc689fbc05c907.png" alt="image-20230901151720798" style="zoom:80%;" />
...
![image-20230901153517526](https://img-blog.csdnimg.cn/d35fffa994fd42a9adfeb74b5d75bd56.png)

当我们尝试通过软链接文件去查看指向文件的内容的时候，就会凸显出差异：

失效的软链接文件`10_2.txt`指向了一个不存在的文件，读取不出来

![image-20230901153642964](https://img-blog.csdnimg.cn/f14fe214d6054fe48cdcfc8b6fb0df85.png)
...
![image-20230901154605717](https://img-blog.csdnimg.cn/40cbf97843e14491a681d0bcc2d23380.png)
...
<img src="https://img-blog.csdnimg.cn/c5903de52c9f427cb86bdbbbf1257859.png" alt="image-20230901155847954" style="zoom:80%;" />

现在我分别在soft目录中用三种方式创建软链接文件，名字，绝对路径和相对路径：

<img src="https://img-blog.csdnimg.cn/926b2f9c39e145c48f5cf8d3a926900d.png" alt="image-20230901160019983" style="zoom:80%;" />

我们现在分别打开这三个文件来看下能否读出对应内容呢？

2和3成功了，1失败了，这已经够明显了，**存储的是路径，访问他们的时候就是通过读取他们存储的路径内容去查找的，绝对路径从根目录开始找，相对路径就从该文件的目录开始找**

<img src="https://img-blog.csdnimg.cn/55265ca82b7d49259c0181a583b9df2c.png" alt="image-20230901160110538" style="zoom:80%;" />

我们同样可以查看文件存储的大小来看一下他们是否存储的是路径，我就不写程序去readlink了

<img src="https://img-blog.csdnimg.cn/bcc2bc4e5fa0406288b50b3dae4599ff.png" alt="image-20230901160316403" style="zoom:80%;" />
...
![image-20230901161513319](https://img-blog.csdnimg.cn/cd5aa46473214651b3b48ce769ede366.png)
...
![image-20230905200000664](https://img-blog.csdnimg.cn/9c37e724c997490bb1899d6f66da1a16.png)
...
- 行缓冲
  ...
  **当流涉及一个终端的时候，就是标准输入和标准输出，通常对应的就是行缓冲**
  ...
  ![image-20230906145510209](https://img-blog.csdnimg.cn/23d69483b1664cfc86c504c84aafac07.png)
...
<img src="https://img-blog.csdnimg.cn/2f66442948814792946bb9d181006739.png" alt="image-20230907145440431" style="zoom:80%;" />
...
![image-20230907150903812](https://img-blog.csdnimg.cn/11cf811ae84847599aff4ca8b8b45663.png)
...
![image-20230907151214997](https://img-blog.csdnimg.cn/42e360cecb7549c69489dc8d10cb524c.png)
...
![image-20230907151444480](https://img-blog.csdnimg.cn/06df5ad4f5da4c3ab061dcb2271953b8.png)
...
<img src="https://img-blog.csdnimg.cn/59ee39f0f9724e5aa9f7d579eae550e4.png" alt="image-20230907152718910" style="zoom:80%;" />

书上还给出了一些注意事项，如下：

<img src="https://img-blog.csdnimg.cn/907774e85b3c43b1ac0e7e42dde2c32c.png" alt="image-20230907153412560" style="zoom:80%;" />
...
![image-20230907153827144](https://img-blog.csdnimg.cn/dd2d43abec904da0ad50e4c30e9d3085.png)

### 其他注意事项(了解)

我就放书上的图了

<img src="https://img-blog.csdnimg.cn/590589955d5746d9b8b2adf850563175.png" alt="image-20230907154448173" style="zoom:80%;" />

<img src="https://img-blog.csdnimg.cn/f23b953ad1d1418691b1f3e5a87d49c6.png" alt="image-20230907154453879" style="zoom:80%;" />
...
![image-20230907170621432](https://img-blog.csdnimg.cn/77f742b2a34943328ccc1e42412aec03.png)

读取错误

![image-20230907170649491](https://img-blog.csdnimg.cn/2343fd7af484436197a33ee121c059fa.png)
...
![image-20230907171454664](https://img-blog.csdnimg.cn/7b314e59d6e74ef0822677225fa04f66.png)
...
![image-20230907173421891](https://img-blog.csdnimg.cn/632afa4a655247b08d2c672523956eb7.png)
...
![image-20230909093226773](https://img-blog.csdnimg.cn/eb6268918a474f20b3cacde0512d0a6b.png)
...
![image-20230909095214017](https://img-blog.csdnimg.cn/627efb93fd8843e5a890aeaad98270ac.png)
...
- 有两种常见的用法：
  ...
  - **读或者写一个二进制数组**
    ...
    ![image-20230909103740225](https://img-blog.csdnimg.cn/55055847ffb34948ac97855a175ae34d.png)

  - **读或者写一个结构**

    ![image-20230909103759521](https://img-blog.csdnimg.cn/ceb29227d7784f94b0ebe68bedeabf5b.png)
...
![image-20230909105848043](https://img-blog.csdnimg.cn/bab64656bba946038bd2f4a3916d54e1.png)
...
![image-20230909110026630](https://img-blog.csdnimg.cn/26de8541c72a45fcb0a1627822ef7193.png)
...
<img src="https://img-blog.csdnimg.cn/4703a91aedf34935bdc9fa657daae926.png" alt="image-20230909110812555"  />
...
![image-20230909113626829](https://img-blog.csdnimg.cn/2be793fbe560450c80add5b4234c3397.png)
