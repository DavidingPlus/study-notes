...
### 11.IP地址转换（字符串ip-整数 ，主机、网络字节序的转换）
...
int listen(int sockfd, int backlog); // /proc/sys/net/core/somaxconn
    ...
    - 参数：
        ...
        - backlog : 未连接的和已经连接的和的最大值，也就是总共可以连接总数的最大值
...
- 另外，我想谈谈关于TCP和UDP双方的文件描述符，TCP里面就是connect_fd，UDP里面就是socket_fd
  ...
  **他们是不相同的，如何理解？**
  **TCP通信双方用于通信的文件描述符需要建立连接，虽然他们两个不同，在我们一般的认知当中肯定也是不同的，因为是在两个程序中分别通过accept()和socket()拿到的，不同就意味着他们有自己的读写缓冲区，他们中间是用类似于TCP管道建立连接的，这样就比较形象了**
  ...
  **他们是相同的，我不能理解，目前为止也没办法真正弄明白，我只能猜测可能是于UDP没有办法建立类似于TCP的通道连接吧；因此在这里这个承载着UDP通信重任的文件描述符的引用计数就不为1了，上次遇到这种情况还是TCP多进程的父子进程当中，可以参照前面笔记**
